...
import LineGraph from "../Graphs/LineGraph/page";
import { Card, CardHeader, CardTitle, CardContent } from "@/components/ui/card";
import { Input } from "@/components/ui/input";
import { Button } from "@/components/ui/button";
import { ScrollArea } from "@/components/ui/scroll-area";
import PieChart from '../Graphs/Pie/page';
import {
  ArrowLeft,
  Upload,
  ChevronDown,
...
import BarGraph from '../Graphs/Bargraphs/page';
import {
  Dialog,
  DialogContent,
  DialogHeader,
...
const pastMeetings = [
  {
    ...
    transcription: [
      {
        text: "The new product features are impressive.",
        emotion: 0.7,
        logic: 0.8,
      },
      {
        text: "We need to focus on our target audience.",
        emotion: 0.2,
        logic: 0.9,
      },
      {
        text: "The pricing strategy might be too aggressive.",
        emotion: -0.3,
        logic: 0.5,
      },
    ],
    ...
    notableInsights: [
      {
        summary: "New product features well-received",
        details:
          "The team expressed enthusiasm about the innovative features of the new product.",
      },
      {
        summary: "Concerns about pricing strategy",
        details:
          "There were discussions about the pricing strategy being potentially too aggressive for the target market.",
      },
    ],
  },
  {
    ...
    transcription: [
      {
        text: "Several customers reported issues with the UI.",
        emotion: -0.6,
        logic: 0.7,
      },
      {
        text: "We need to improve our response time.",
        emotion: -0.4,
        logic: 0.8,
      },
      {
        text: "The new feature has received positive feedback.",
        emotion: 0.8,
        logic: 0.9,
      },
    ],
    ...
    notableInsights: [
      {
        summary: "UI issues reported by customers",
        details:
          "Multiple customers have reported difficulties navigating the user interface, suggesting a need for UX improvements.",
      },
      {
        summary: "Positive reception of new feature",
        details:
          "The recently launched feature has been well-received by users, indicating a successful product addition.",
      },
    ],
  },
  {
    ...
    transcription: [
      {
        text: "Our team has exceeded expectations this quarter.",
        emotion: 0.9,
        logic: 0.8,
      },
      {
        text: "We need to address the high turnover in the sales department.",
        emotion: -0.5,
        logic: 0.7,
      },
      {
        text: "The new training program has shown promising results.",
        emotion: 0.6,
        logic: 0.8,
      },
    ],
    ...
    notableInsights: [
      {
        summary: "Exceptional team performance",
        details:
          "The team has surpassed expectations in their quarterly performance, showing strong growth and productivity.",
      },
      {
        summary: "High turnover in sales department",
        details:
          "There is a concerning trend of high employee turnover in the sales department that needs to be addressed.",
      },
    ],
  },
];
...
const generateRandomData = () => {
  ...
  return {
    feelings: Array(3)
      .fill(null)
      .map(() => feelings[Math.floor(Math.random() * feelings.length)]),
    topics: Array(3)
      .fill(null)
      .map(() => topics[Math.floor(Math.random() * topics.length)]),
  };
};

const generatePatternDetection = (emotion, logic) => {
  const sentences = [
    `The sentiment analysis reveals a ${
      emotion > 0 ? "positive" : "negative"
    } emotional response with a strength of ${Math.abs(emotion).toFixed(2)}.`,
    `The logical assessment of the statement is ${
      logic > 0 ? "favorable" : "unfavorable"
    } with a confidence of ${Math.abs(logic).toFixed(2)}.`,
    `There appears to be a ${
      Math.abs(emotion - logic) < 0.3 ? "strong" : "weak"
    } correlation between emotional and logical responses.`,
    `This pattern suggests ${
      emotion * logic > 0 ? "alignment" : "conflict"
    } between emotional and rational perspectives on the subject.`,
  ];
  return sentences;
};
  ...
  const handleMouseMove = (e) => {
    if (isSelecting && textRef.current) {
      ...
      const textElement = elements.find((el) =>
        el.classList.contains("text-item")
      );
      if (textElement) {
        const index = parseInt(
          textElement.getAttribute("data-index") || "-1",
          10
        );
        if (
          index !== -1 &&
          !selectedItems.some((item) => item.text === data[index].text)
        ) {
          setSelectedItems((prev) => [
            ...prev,
            { ...data[index], randomData: generateRandomData() },
          ]);
        }
      }
    }
              ...
              {dataSet.map((item, index) => (
                <div key={index} className="p-2 border-b last:border-b-0">
                  ...
                  <p className="text-sm text-gray-500">
                    Emotion: {item.emotion.toFixed(2)}, Logic:{" "}
                    {item.logic.toFixed(2)}
                  </p>
                </div>
              ))}
            </ScrollArea>
    ...
    <>
      <div className="container mx-auto p-4 bg-gradient-to-br from-purple-50 to-blue-50 min-h-screen">
                  ...
                  {data.map((item, index) => (
                    <div
                      ...
                      className={`text-item p-2 cursor-pointer rounded-md transition-colors duration-200 ${
                        selectedItems.some((i) => i.text === item.text)
                          ? "bg-purple-100"
                          : "hover:bg-gray-100"
                      }`}
                      data-index={index}
                      ...
                      {selectedItems.map((item, index) => (
                        ...
                        >
                          ...
                          <div className="text-sm mt-2">
                            ...
                            {item.randomData.feelings.map((feeling, i) => (
                              <Badge
                                key={i}
                                variant="outline"
                                className="mr-1 mb-1"
                          ...
                          <div className="text-sm mt-2">
                            ...
                            {item.randomData.topics.map((topic, i) => (
                              <Badge
                                key={i}
                                variant="outline"
                                className="mr-1 mb-1"
                    ...
                    >
                      ...
                      <AccordionContent>
                        <div className="p-2 rounded-md bg-white">
                          ...
                          <div className="mt-2">
                            <strong>Feelings:</strong>{" "}
                            {meeting.feelings.map((feeling, i) => (
                              <Badge
                                key={i}
                                variant="secondary"
                                className="mr-1 mb-1"
        ...
        <Dialog
          ...
          onOpenChange={() => setSelectedPastMeeting(null)}
        >
          <DialogContent className="max-w-4xl">
            ...
            {selectedPastMeeting &&
              renderMeetingCard(
                "Meeting Analysis",
                selectedPastMeeting.transcription
              )}
          </DialogContent>
        </Dialog>
      </div>
      ...
      <div className="flex justify-center m-auto gap-5 mb-4">
...
<button
  ...
  onClick={() => setShowPieGraph(prev => !prev)}
>
  Show in Pie Graphs
</button>
...
{showBarGraph && 
   ...
   <div className="flex justify-center m-auto">
        <div className="flex justify-center items-center w-[90%] text-white p-4 bg-black h-[500px]">
        <div className="container">
      <h2 className="text-xl font-semibold mb-4">Emotion and Logic Trends</h2>
...
<div className="container">
<h2 className="text-xl font-semibold mb-4">Emotion and Logic Distribution</h2>
