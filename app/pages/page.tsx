...
const generatePatternDetection = (emotion: number, logic: number) => {
  const sentences = [
...
export default function Page() {
  ...
  const [selectedItems, setSelectedItems] = useState<(typeof initialData[0] & { randomData: ReturnType<typeof generateRandomData> })[]>([])
  const [isSelecting, setIsSelecting] = useState(false)
  const [selectedPastMeeting, setSelectedPastMeeting] = useState<typeof pastMeetings[0] | null>(null)
  ...
  const handleTextClick = (item: typeof initialData[0]) => {
  ...
  const renderDistributionGraph = (dataSet: typeof initialData, height: string = "h-[400px]") => (
  ...
  const renderMeetingCard = (title: string, dataSet: typeof initialData) => (
    <Card>
      <CardHeader>
